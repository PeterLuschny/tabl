...
from tabl import GetOEISdata, GenerateAllCsvFiles, AllCsvToHtml, SaveExtendedTables, CrossReferences 

'''
DO NOT USE this script if you only want to update tabl.py.
If you want to update table.py after you have added a new
triangle implementation just run "src/_tablmake.py".
As a normal user this is all you want and is quick.

On the other hand this script updates all data related to the 
...
def update() -> None:
    ...
    GenerateAllCsvFiles()
    AllCsvToHtml()
    SaveExtendedTables() # or the short version SaveTables()
    CrossReferences()
