...
def PrintViews(
    T: tgen, rows: int = 7, cono: int | None = None, verbose: bool = True
) -> None:
    print("# " + T.__name__)
...
def _abel(n: int) -> list[int]:
    if n == 0:
        return [1]
    return [
        binomial.val(n - 1, k - 1) * n ** (n - k) if k > 0 else 0 for k in range(n + 1)
    ]
...
def _euler_sec(n: int) -> list[int]:
    if n == 0:
        return [1]
    row: list[int] = [
        binomial.val(n, k) * _euler_sec(n - k)[0] if k > 0 else 0 for k in range(n + 1)
    ]
    if n % 2 == 0:
        row[0] = -sum(row[2::2])
    return row
...
def _euler_tan(n: int) -> list[int]:
    row: list[int] = [
        binomial.val(n, k) * _euler_tan(n - k)[0] if k > 0 else 0 for k in range(n + 1)
    ]
    if n % 2 == 1:
        row[0] = -sum(row[2::2]) + 1
    return row
