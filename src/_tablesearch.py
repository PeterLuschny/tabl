...
'''
def ess_equal(s: list[int], t: list[int]) -> tuple[int, int]:
...
def read_seqdata() -> list[list]: 
    inpath = 'C:\\Users\\User\\PythonTabl\\tabl\\data\\oeis_data.csv'
...
def search_oeis(A: list[int]) -> None:
    Seqs = read_seqdata()
    candidates = []
    for seq in Seqs:
